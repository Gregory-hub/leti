...
class Algorithm
{
	...
	// 	private int DecodeSymbol(ref Model model, Dictionary<int, int> frequencies, ref int i, string text)
	// 	{
	// 		model.Encoder.Freqs = frequencies;
	// 		model.Encoder.Count = frequencies.Sum(freq => freq.Value);
	// 		char sym = model.Encoder.DecodeNextSymbol(text);

	// 		if (sym == '\0') return sym;

	// 		model.Encoder.UpdateDecode(text, ref i);
	// 		return sym;
	// 	}
	...
	// 		string text_decoded = "";
	// 		Model model = new Model(model_order);
	// 		int i = model.Encoder.InitDecoder(text, symbols, out int precision);
	...
	// 				if (model.Contexts[order].ContainsKey(context))
	// 				{
	// 					sym = DecodeSymbol(ref model, model.Contexts[order][context].Frequences, ref i, text);
	// 					if (sym == ESC)
	// 					{
	// 						order--;
	// 						if (order == -1) break;
	...
	// 				else
	// 				{
	// 					sym = DecodeSymbol(ref model, minus_one_frequencies, ref i, text);
